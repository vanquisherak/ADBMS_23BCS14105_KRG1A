...
To solve this problem, the company needs an automated database-driven solution that can instantly return the count of employees by gender through a stored procedure that:1. Create a PostgreSQL stored procedure that:
...
Whenever a customer places an order, the system must:1. Verify stock availability for the requested product and quantity.
2. If sufficient stock is available:	- Log the order in the sales table with the ordered quantity and total price.
	...
	- Display a real-time confirmation message: "Product sold successfully!“3. If there is insufficient stock, the system must:
